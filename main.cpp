...
static const string BASE_URL = "https://api-am-six.vercel.app";
static const string API_KEY = "XVoid-ashar";
...
string encode(CURL* curl, const string& value) {
    char* p = curl_easy_escape(curl, value.c_str(), (int)value.length());
    if (!p) return "";
    string result(p);
    curl_free(p);
    return result;
...
string getRequest(const string& url) {
    ...
    if (!curl) return R"({"status":false,"message":"CURL init failed"})";
